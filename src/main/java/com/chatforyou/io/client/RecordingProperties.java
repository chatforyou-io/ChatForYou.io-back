...
/**
 * See
 * {@link OpenVidu#startRecording(String, RecordingProperties)}
 */
public class RecordingProperties {
	...
	/**
	 * Builder for {@link RecordingProperties}
	 */
	public static class Builder {
		 ...
		 * {@link OutputMode#COMPOSED} or
		 * {@link OutputMode#COMPOSED_QUICK_START} for
		 * a single archive in a grid layout or {@link OutputMode#INDIVIDUAL}
		 ...
		 * have effect for {@link OutputMode#COMPOSED}
		 * or {@link OutputMode#COMPOSED_QUICK_START}
		 ...
		 * {@link OutputMode#COMPOSED} or
		 * {@link OutputMode#COMPOSED_QUICK_START}
		 ...
		 * for {@link OutputMode#COMPOSED} or
		 * {@link OutputMode#COMPOSED_QUICK_START}
		 * recordings with {@link RecordingProperties#hasVideo()} to true. Property
		 * ignored for INDIVIDUAL recordings and audio-only recordings. For
		 * {@link OutputMode#INDIVIDUAL} all
		...
		/**
		 * If setting
		 * {@link Builder#recordingLayout(RecordingLayout)}
		 ...
		 * {@link OutputMode#INDIVIDUAL} recordings.
		 ...
		 * for {@link OutputMode#INDIVIDUAL}
	...
	protected RecordingProperties(String name, Boolean hasAudio, Boolean hasVideo, Recording.OutputMode outputMode,
		...
		if ((Recording.OutputMode.COMPOSED.equals(this.outputMode) || Recording.OutputMode.COMPOSED_QUICK_START.equals(this.outputMode))
				&& this.hasVideo) {
	...
	/**
	 * Defines the mode of recording: {@link OutputMode#COMPOSED} or
	 * {@link OutputMode#COMPOSED_QUICK_START} for
	 * a single archive in a grid layout or {@link OutputMode#INDIVIDUAL}
	 * for one archive for each stream.<br>
	 * <br>
	 *
	 * Default to {@link OutputMode#COMPOSED OutputMode.COMPOSED}
	 */
	...
	/**
	 * If {@link RecordingProperties#recordingLayout()} is
	...
	public JsonObject toJson() {
		...
		if ((Recording.OutputMode.COMPOSED.equals(outputMode) || Recording.OutputMode.COMPOSED_QUICK_START.equals(outputMode))
				&& hasVideo) {
		...
		if (Recording.OutputMode.INDIVIDUAL.equals(outputMode)) {
	 ...
	 * @hidden
	 */
	public static Builder fromJson(Map<String, ?> params, RecordingProperties defaultProps)
		...
		Recording.OutputMode outputModeFinal = null;
		...
		Recording.OutputMode outputModeDefault = null;
		...
		if (outputModeStringParam != null) {
			try {
				...
				if (defaultProps != null && Recording.OutputMode.COMPOSED_QUICK_START.equals(defaultProps.outputMode())
						&& Recording.OutputMode.COMPOSED.equals(outputModeFinal)) {
					outputModeFinal = Recording.OutputMode.COMPOSED_QUICK_START;
				} else if (defaultProps != null && Recording.OutputMode.COMPOSED.equals(defaultProps.outputMode())
						&& Recording.OutputMode.COMPOSED_QUICK_START.equals(outputModeFinal)) {
		...
		} else if (Recording.OutputMode.INDIVIDUAL.equals(outputModeFinal)) {
		...
		if (Recording.OutputMode.INDIVIDUAL.equals(outputModeFinal)) {
	 ...
	 * @hidden
	 */
	public final static boolean IS_COMPOSED(Recording.OutputMode outputMode) {
